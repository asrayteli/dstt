...
設計方針:
  - 取り込みは IMAP UID で冪等化（同じ UID は二重保存しない）。1回の取り込み件数は
    上限付きで、未取得分は次回以降のポーリングで取りこぼさず順次取り込む。
  - 既知の制約: サーバー側の UIDVALIDITY が変わると UID が振り直されるため、稀に
    取りこぼし/重複が起こりうる（簡易メーラーとして許容。重複は (mailbox,uid)
    一意制約で抑止）。
