(function(root) {
  ...
  function openCell(day, memberId) {
    ...
    const body = `<div class="cloud-large-cell-editor">
      ...
      <details class="cloud-fold"${entry.time_override || entry.bind_override_minutes != null || entry.comment ? ' open' : ''}><summary class="cloud-fold-summary">詳細・コメント</summary><div class="cloud-fold-content cloud-large-detail-grid"><label class="cloud-field"><span>始業上書き ${help('この現場の勤務が1つのときは、その勤務コードの時刻より優先します。他の帳簿から同期された勤務だけの日は、この時間を設定すると拘束・労働時間に集計されます。')}</span><input class="cloud-input" type="time" data-cell-start value="${esc(time.start || '')}"></label><label class="cloud-field"><span>終業上書き</span><input class="cloud-input" type="time" data-cell-end value="${esc(time.end || '')}"></label><label class="cloud-field"><span>所定拘束（時間:分） ${help('この日の残業判定の基準です。例: 9:30')}</span><input class="cloud-input" inputmode="numeric" placeholder="9:30" data-cell-bind value="${esc(durationText(entry.bind_override_minutes))}"></label><label class="cloud-field cloud-field-wide"><span>コメント</span><textarea class="cloud-input" rows="3" data-cell-comment>${esc(entry.comment || '')}</textarea></label></div></details></div>`;
