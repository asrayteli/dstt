...
- 受信箱（期日なし）は、詳細パネルで期日を消したタスクの置き場になる。
...
- 副作用: 期日を指定しなかったタスクも、翌日以降は「期限切れ」に入る。
...
- プロジェクトの並び順（`sort_order`）は全員で共有するため、変更は作成者・管理者のみ。
...
- アーカイブ時に Google カレンダーの予定を消したら、タスク側の参照も外す。
