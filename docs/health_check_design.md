...
| 二次検査ToBell | 推奨日の前日にタスク化し、**推奨日当日9:00** に通知。通知種別は利用者ごとにON/OFF可 |
...
### 3.4 アクセス権（DSTTのアクセス権に同期）

健康情報＝要配慮個人情報のため厳格に運用する。**健診PLUS独自の営業所付与は廃止**し、
DSTTのアクセス権と完全に同期する（§11 で改定）。

- **営業所スコープ**：閲覧・作成・編集できるのは「DSTTで本人に付与された営業所」のみ
  （`access_control.user_office_codes`）。健診PLUS側での営業所付与は行わない。
- **DSTT管理者（`is_admin`）**：全営業所を閲覧・編集できる。
- **健診PLUS管理者**：`static/health_check/permissions.json` の `admins` に登録された人。
  ただし権限は**自分の営業所の管理者**に限る（他営業所へはアクセス不可）。
  自営業所の「健康診断担当」を設定できる。管理者の指定・解除は **DSTT管理者のみ**。
- 健診PLUS管理者であること自体は営業所アクセスを増やさない（スコープは常にDSTT準拠）。
...
## 11. アクセス権の改定（DSTTアクセス権との同期）

健康情報を扱うため、営業所アクセスを健診PLUS独自に付与する旧方式を廃止し、
DSTTのアクセス権に一本化した。

| 役割 | 閲覧・作成・編集できる営業所 | 管理操作（健康診断担当の設定） | 健診PLUS管理者の指定 |
|---|---|---|---|
| DSTT管理者 | 全営業所 | 全営業所 | 可 |
| 健診PLUS管理者 | DSTTで本人に付与された営業所のみ | 自分の営業所のみ | 不可（DSTT管理者のみ） |
| 一般利用者 | DSTTで本人に付与された営業所のみ | 不可 | 不可 |

- 営業所スコープは `access_control.user_office_codes(user)`（DSTT付与）に同期する。
  `permissions.json` の `user_offices`（旧・独自付与）は廃止し、読み込み時に無視する。
- バックエンド判定:
  - `is_dstt_admin()` … DSTT全体管理者。全営業所の閲覧・編集を許可。
  - `get_user_offices()` … DSTT管理者は全営業所、それ以外は DSTT付与の営業所のみ。
  - `has_office_access()` … レコードの閲覧・作成・編集の可否。
  - `is_health_admin()` … 健診PLUS管理者か（DSTT管理者を含む）。営業所アクセスは増やさない。
  - `can_admin_office()` … 健康診断担当の設定可否（DSTT管理者＝全、健診PLUS管理者＝自営業所）。
- 全営業所横断の編集履歴閲覧・ツール全体設定の変更は DSTT管理者のみ。
