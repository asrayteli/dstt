...
- ログイン、管理者、ツール別アクセス制御（ツール間API専用許可を含む）
...
### ツール別アクセス権とツール間API専用許可

要許可（sensitive）ツールの個別付与には「通常許可」と「ツール間API専用許可」の 2 段階があります。

- **通常許可**: 従来どおり。ツール本体の画面も全 API も使えます。
- **ツール間API専用許可**: 提供元ツール本体は開けず、他ツールが使うために公開している
  参照専用 API だけを許します。たとえば「社員名簿PLUS は見せないが、ShifterSync の
  社員候補サーチだけは使わせる」という配り方ができます。単独では何も参照できず、
  利用側ツール（ShifterSync など）の通常許可との AND で初めて効きます。
  返却項目も社員番号・氏名・営業所名・役職名に絞られ、住所などは返しません。
